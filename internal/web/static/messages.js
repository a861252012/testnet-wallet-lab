...
window.FlowMessages = {
  "存入與提領": ["Deposit and withdraw", "存入与提取"],
  "扣款錢包": ["Source wallet", "扣款钱包"],
  "收款錢包": ["Receiving wallet", "收款钱包"],
  "合約依錢包地址記錄存款；使用同一個地址，就共用同一筆存款。": ["Deposits belong to a wallet address. Anyone using the same address shares its deposit balance.", "合约按钱包地址记录存款；使用同一个地址，就共用同一笔存款。"],
  "請使用「存入」操作，勿直接轉帳至合約地址。": ["Use Deposit; do not send ETH directly to the contract address.", "请使用“存入”操作，勿直接转账至合约地址。"],
  "存款箱": ["Vault", "存款箱"],
  "測試 ETH 存款箱": ["Test ETH vault", "测试 ETH 存款箱"],
  "存入與提領測試 ETH，合約為每個地址獨立記帳。": ["Deposit and withdraw test ETH. The contract tracks each address separately.", "存入与提取测试 ETH，合约为每个地址独立记账。"],
  "重新整理存款箱": ["Refresh vault", "刷新存款箱"],
  "載入存款箱狀態中…": ["Loading vault…", "正在加载存款箱…"],
  "存款狀態": ["Vault balance", "存款状态"],
  "錢包可用餘額：—": ["Wallet balance: —", "钱包可用余额：—"],
  "我的合約存款：—": ["My vault balance: —", "我的合约存款：—"],
  "金額（ETH）": ["Amount (ETH)", "金额（ETH）"],
  "存入會將錢包中的測試 ETH 轉入存款箱合約；提領則將合約中的存款轉回本錢包。": ["Deposits move test ETH into the vault. Withdrawals return your deposit to this wallet.", "存入会将钱包中的测试 ETH 转入存款箱合约；提取则将存款转回本钱包。"],
  "存入": ["Deposit", "存入"],
  "提領": ["Withdraw", "提取"],
  "最近操作": ["Recent operations", "最近操作"],
  "存入存款箱": ["Deposit to vault", "存入存款箱"],
  "從存款箱提領": ["Withdraw from vault", "从存款箱提取"],
  "尚無存款箱操作紀錄。": ["No vault operations yet.", "尚无存款箱操作记录。"],
  "收據顯示執行成功；存款餘額請至存款箱重新查詢。": ["Receipt confirms execution; refresh the vault to check your deposit balance.", "收据显示执行成功；请到存款箱刷新存款余额。"],
  "正在查詢存款箱…": ["Checking vault…", "正在查询存款箱…"],
  "存款箱尚未啟用。": ["The vault is not enabled yet.", "存款箱尚未启用。"],
  "錢包可用餘額：${balance.eth} ETH": ["Wallet balance: ${balance.eth} ETH", "钱包可用余额：${balance.eth} ETH"],
  "我的合約存款：${vault.balance} ETH": ["My vault balance: ${vault.balance} ETH", "我的合约存款：${vault.balance} ETH"],
  "僅支援 Ethereum Sepolia；存款不產生利息，存入與提領皆需支付 Gas。": ["Ethereum Sepolia only. No interest; deposits and withdrawals require gas.", "仅支持 Ethereum Sepolia；存款不产生利息，存入与提取均需支付 Gas。"],
  "無法取得最新存款，請重新查詢。": ["Could not load the latest vault balance. Try refreshing.", "无法取得最新存款，请重新查询。"],
  "存款箱合約": ["Vault contract", "存款箱合约"],
  "提領說明": ["Withdrawal details", "提取说明"],
  "提領金額回到本錢包，錢包另付 Gas；最多扣除欄位僅為費用上限。": ["Withdrawals return ETH to this wallet. The maximum debit shown covers gas only.", "提取金额返回本钱包，钱包另付 Gas；最多扣除字段仅为费用上限。"],
