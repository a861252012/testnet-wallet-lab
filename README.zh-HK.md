...
**以 Go 開發的測試網錢包，探索交易可靠性與故障復原。**

FlowLedger 涵蓋費用預覽、本地簽署、持久化、廣播、收據核對與最終確定性觀測。項目著重後端常見的交易難題：RPC 回應遺失、並發重試、程序中止與鏈重組。

[三分鐘展示](docs/demo-script.md) · [鏈上驗收紀錄](docs/onchain-acceptance-2026-09-15.md) · [故障復原驗證](docs/process-recovery.md)

## 工程重點

- **先保存，再廣播**：先寫入簽署後的交易字節與雜湊值；復原時沿用同一筆已簽署交易。
- **重試去重**：同一賬戶、網絡與 quote ID 沿用既有交易日誌。不同報價仍須由上游付款服務以業務識別碼防止重複付款。
...
[2026-09-15 驗收紀錄](docs/onchain-acceptance-2026-09-15.md)記載**五個測試網共 13 筆成功發送交易**。這是歷史快照，不代表所有功能或目前網絡狀態均已驗證；Mock 測試另屬不同證據。

[ERC-4337 元件](internal/wallet/erc4337)包含 UserOperation 編碼、雜湊值、建構、簽署、Bundler 客戶端及 Mock 測試。目前**尚未整合至錢包介面**，也未驗證已部署的智能賬戶或真實 Bundler 驗收；詳見[模組範圍](PROJECT.md)。

## 本地啟動

先安裝 Docker 與 Docker Compose，再於項目根目錄執行。只有尚無 `.env` 時才建立，已有設定請保留。
...
開啟 [localhost:8090](http://localhost:8090)，作品展示頁為 `/showcase`。預設本地展示不需登入；將 `WALLET_ACCESS_TOKEN` 設為至少 32 個字元可啟用登入。連接埠請維持綁定 `127.0.0.1`，不要透過通道或公開代理對外開放錢包。

1. 建立或還原測試專用錢包，離線妥善備份助記詞。
2. 選擇正確測試網並取得測試資產。選用的本地發幣賬戶需要設定與餘額；複製項目不會附帶資金或憑證。
...
**僅供測試網原型使用，請勿匯入持有真實資產的錢包。** Go 程序會在簽署時處理解密後的金鑰；本項目不是經安全審計的託管服務或公開多用戶錢包。

- 保存完整 `wallet_data` volume，包含交易日誌與封存檔案。只還原助記詞無法恢復交易紀錄或待處理交易的保護機制。
- `docker compose down` 保留 volume；**`docker compose down -v` 會刪除錢包數據。**
- 收據及最終確定性核對依賴設定的 RPC 供應商，尚未實作獨立 RPC 多方共識驗證。
- 收支流水及 CSV 僅涵蓋已索引並驗證的交易，不是完整賬本、所有內部轉帳或獨立餘額對賬。
- 目前展示範圍不包含主網、正式營運就緒保證、SLA 或獨立安全審計。
...
透過公開測試網 RPC 重新核對已發佈的交易證據，不簽署也不廣播：
...
| `internal/wallet/erc4337` | 獨立賬戶抽象化元件 |
