...
Solidity 存款／提領合約搭配 Sepolia 面板，包含每個地址獨立餘額與重入防護。[2026-09-19 驗收紀錄](docs/evidence/vault-sepolia-2026-09-19/REPORT.md)記載公共 Sepolia 部署、Sourcify 原始碼驗證及存提結果；Etherscan 個別驗證仍未完成。紀錄代表當時版本，未設定合約地址的環境仍停用操作。

新增[測試 USDC 付款託管](docs/payment-escrow.md)：付款人付款與放款，收款人可全額退回原付款人。沿用交易日誌與重試流程，提供合約、Go／HTTP 及 Chromium＋模擬 EVM 測試；已在公共 Sepolia 完成付款、放款與退款，見[交易收據與餘額驗收](docs/evidence/escrow-sepolia-2026-09-22/README.md)。
...
介面將交易狀態與依收據整理的收支明細集中在「活動」。診斷入口放在設定；公開共用模式隱藏無法使用的管理操作。EVM、Solana 與 TRON 的地址簿支援搜尋、改名、複製、直接發送及移除後復原，依測試網路儲存在目前瀏覽器；不會跨裝置同步。EVM 收款人選單也包含已建立的錢包。測試幣由按鈕直接申請；EVM／TRON 需設定有庫存的專用發幣帳戶，SOL 使用 Devnet 空投並受上游額度限制。Demo 部署以 TEST_FAUCET_CONFIG 指向 /data/wallet 內權限為 0600 的 JSON 設定檔；發幣私鑰與密碼不可提交至 Git。
