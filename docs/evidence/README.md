...
These tests use temporary wallets, local RPC fixtures or a simulated EVM. Successful local tests establish the behavior of those cases; public-chain receipts are separate evidence. For access policy, consult the [local, protected and shared deployment modes](../architecture.md#transaction-behavior-and-evidence) and [deployment configuration](../deployment.md).
